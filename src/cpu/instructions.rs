...
pub fn ld<D: Copy, S: Copy>(&mut self, &mut bus: &mut Peripherals, dst: D, src: S)
where Self: IO8<D> + IO8<S>{
    step!((), {
        0: if let Some(v) = self.read8(bus, src){
            VAL8.store(vm Relaxed);
            go!(1);
        },
...
where Self: IO8<S>{
    if let Some(v) = self.read8(bus, src){
        let (result, carry) = self.regs.a.overflowing_sub(v);//result->演算結果, carry->繰り下がり
...
where Self: IO8<S>{
    step!((), {
        0: if let Some(v) = self.read(bus, src){
...
pub fn rl<S: Copy>(&mut self, bus: &Peripherals, src: S)
where Self: IO8<S>{
    step!((), {
        0: if let Some(v) = self.read8(bus, src){
        ...
        1: if self.write88(bus, src, VAL8.load(Relaxed)).is_some(){
            go!(0);
            self.fetch(bus);
        },
    });
...
pub fn push16(&mut self, bus: &mut Peripherals, val: u16) -> Option<()>{
    step!(None, {
        ...
        2: {
            self.regs.sp = self.regs.sp.wrapping_sub(1);
            buss.write(self.regs.sp, VAL8.load(Relaxed));
            go!(3);
            return None;
        },
        3: return Some(go!(0)),
