...
impl IO16<Reg16> for Cpu{
    ...
    fn write16(&mut self, _: &mut Peripherals, dst: Reg16, val: u16) -> Option<()>{
        Some(match dst{
            ...
            Reg16::HL=>self.regs.write_hl(cal),
...
impl IO8<Indirect> for Cpu{
    ...
    fn write8(&mut self, bus: &Peripherals, dst: Indirect, val: u8) -> Option<()>{
        step!(None, {
            0: {
                match dst{
...
impl IO8<Direct8> for Cpu{
    ...
    fn write8(&mut self, bus: &Peripherals, dst: Direct8, val: u8) -> Option<()>{
        step!(None, {
            0: if let Some(lo) = self.read8(bus, Imm8){
                VLA8.store(lo, Relaxed);
                go!(1);
